...
<a href="{{ route('posts.index') }}" class="btn btn-success mb-5">Ver Todos</a>
    ...
    <section class="articles_list">
        ...
        <small>Criado em: {{ date('d/m/Y H:i'), strtotime($post->createad_at) }}- Editado em: {{ date('d/m/Y H:i'), strtotime($post->updated_at) }}</small>
        <form action="{{ route('posts.forceDelete', ['post' => $post->id])}}" method="POST" class="mt-3">
            @csrf
            @method('DELETE')
